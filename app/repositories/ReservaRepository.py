...
class ReservaRepository:
    ...
    def listar(self):
        return self.db.scalars(select(Reserva)).all()

    def listar_activas(self):
        return self.db.scalars(select(Reserva).where(Reserva.estado.in_(["PENDIENTE", "CONFIRMADA"]))).all()

    def listar_por_cliente(self, idCliente: int):
        return self.db.scalars(select(Reserva).where(Reserva.idCliente == idCliente).order_by(Reserva.fechaInicioReserva.desc())).all()

    def obtener_por_id(self, idReserva: int):
        return self.db.scalar(select(Reserva).where(Reserva.idReserva == idReserva))

    def existe_cruce_reserva(self, idEspacio: int, fechaInicioReserva, fechaFinReserva, idReservaExcluir: int | None = None):
        ...
        if idReservaExcluir is not None:
            consulta = consulta.where(Reserva.idReserva != idReservaExcluir)
        return self.db.scalar(select(consulta.exists())) is True
