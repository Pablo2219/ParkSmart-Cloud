...
class Sector(Base):
    ...
    __table_args__ = (
        Index("UK_Sector_NombreSector", "nombreSector", unique=True),
        Index("IX_Sector_Estado", "estado"),
        Index("IX_Sector_IdProveedor", "idProveedor"),
    )

    idSector: Mapped[int] = mapped_column(BIGINT(unsigned=True), primary_key=True, autoincrement=True)
    idProveedor: Mapped[Optional[int]] = mapped_column(BIGINT(unsigned=True), ForeignKey("proveedor.idProveedor", name="FK_Sector_Proveedor", ondelete="SET NULL", onupdate="CASCADE"), nullable=True)
    nombreSector: Mapped[str] = mapped_column(String(50), nullable=False)
    ...
    estado: Mapped[str] = mapped_column(SqlEnum("ACTIVO", "INACTIVO", "MANTENIMIENTO"), nullable=False, server_default=text("'ACTIVO'"))
